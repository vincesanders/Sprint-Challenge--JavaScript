...
const universities = graduates.map(grad => grad = grad.university);
...
const contactInfo = graduates.map(grad => grad = `${grad.first_name} ${grad.email}`);
...
const lowCaseAnimalNames = zooAnimals.map(animal => animal = animal.animal_name.toLowerCase());
